...
func InboundHandler(c *gin.Context) {
    ...
    // 调用service层的Inbound函数执行入库业务逻辑
    // 如果入库过程中发生错误（如数据库操作失败等），返回500错误
    if err := service.Inbound(req); err != nil {
...
func PickupHandler(c *gin.Context) {
    ...
    // 调用service层的Pickup函数执行取件业务逻辑
    // 如果取件失败（如取件码错误、包裹不存在、包裹已被取走等），返回409错误
...
func GetMyParcelHandler(c *gin.Context) {
    // 从URL查询参数中获取手机号
    // 例如：GET /api/v1/parcels?phone=13800138000
    phone := c.Query("phone")
    ...
    if phone == "" {
        c.JSON(http.StatusBadRequest, gin.H{
            "error": "phone parameter is required", // 手机号参数必填
        })
        return
    }
